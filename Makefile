...
help:
	...
	@echo "$(YELLOW)🏭 FÁBRICA EN LA NUBE - BUILD & PUSH A DOCKER HUB (PROD 🚀)$(RESET)"
	@echo "  make build-and-push           → Compilar y subir todas las imágenes de PRODUCCIÓN (:prod)"
	@echo "  make build-backend            → Compilar y subir Backend PROD"
	@echo "  make build-admin              → Compilar y subir Frontend Admin PROD (Inyecta .env.prod)"
	@echo "  make build-public             → Compilar y subir Frontend Público PROD (Inyecta .env.prod)"
	@echo ""

	@echo "$(YELLOW)🏭 FÁBRICA EN LA NUBE - BUILD & PUSH A DOCKER HUB (DEV 🧪)$(RESET)"
	@echo "  make build-dev-and-push       → Compilar y subir todas las imágenes de DESARROLLO (:dev)"
	@echo "  make build-dev-backend       → Compilar y subir Backend DEV"
	@echo "  make build-dev-admin         → Compilar y subir Frontend Admin DEV (Inyecta .env.dev)"
	@echo "  make build-dev-public        → Compilar y subir Frontend Público DEV (Inyecta .env.dev)"
	@echo ""

	@echo "$(YELLOW)🚀 DESPLIEGUE Y ACTUALIZACIÓN EN VPS (PRODUCCIÓN 🚀)$(RESET)"
	@echo "  make prod                     → Levantar entorno de producción existente"
	@echo "  make prod-update-all          → Pull de imágenes frescas y recreación completa de PROD"
	@echo "  make update-proxy-prod        → Recrear Proxy de Producción sin caídas"
	@echo "  make update-all               → Actualizar y recrear todos los servicios de PROD en VPS"
	@echo "  make update-backend           → Actualizar Backend de PRODUCCIÓN (Atómico)"
	@echo "  make update-admin             → Actualizar Frontend Admin de PRODUCCIÓN (Atómico)"
	@echo "  make update-public            → Actualizar Frontend Público de PRODUCCIÓN (Atómico)"
	@echo ""

	@echo "$(YELLOW)🧪 DESPLIEGUE Y ACTUALIZACIÓN EN VPS (DESARROLLO 🧪)$(RESET)"
	@echo "  make dev                      → Levantar entorno de desarrollo existente"
	@echo "  make dev-update-all           → Pull de imágenes :dev y recreación completa de DEV"
	@echo "  make update-dev-all           → Actualizar todos los servicios de DEV en VPS"
	@echo "  make update-dev-backend       → Actualizar Backend de DESARROLLO (Atómico)"
	@echo "  make update-dev-admin         → Actualizar Frontend Admin de DESARROLLO (Atómico)"
	@echo "  make update-dev-public        → Actualizar Frontend Público de DESARROLLO (Atómico)"
	@echo ""

	@echo "$(YELLOW)🔄 REINICIO DE SERVICIOS (PROD & DEV)$(RESET)"
	...
	@echo "  make restart-dev              → Reiniciar todos los servicios de Desarrollo"
	@echo "  make restart-dev-backend      → Reiniciar Backend desarrollo"
	@echo "  make restart-dev-db           → Reiniciar PostgreSQL desarrollo"
	@echo "  make restart-dev-admin        → Reiniciar Frontend Admin desarrollo"
	@echo "  make restart-dev-public       → Reiniciar Frontend Público desarrollo"
	@echo ""
	...
	@echo "  make inspect-backend          → Inspección profunda del contenedor Backend PROD"
	@echo "  make inspect-db               → Inspección profunda de la Base de Datos PROD"
	@echo "  make inspect-admin            → Inspeccionar contenedor Admin PROD"
	@echo "  make inspect-public           → Inspeccionar contenedor Público PROD"
	@echo "  make config                   → Render final de la configuración Compose de PROD"
	@echo "  make validate                 → Validar sintaxis del docker-compose.prod.yml"
	@echo ""

	@echo "$(YELLOW)📊 MONITOREO, INSPECCIÓN Y LOGS (DESARROLLO 🧪)$(RESET)"
	...
	@echo "  make inspect-dev-backend      → Inspeccionar contenedor Backend DEV"
	@echo "  make inspect-dev-db           → Inspeccionar contenedor DB DEV"
	@echo "  make inspect-dev-admin       → Inspeccionar contenedor Admin DEV"
	@echo "  make inspect-dev-public      → Inspeccionar contenedor Público DEV"
	@echo "  make config-dev               → Render final de la configuración Compose de DEV"
	@echo "  make validate-dev             → Validar sintaxis del docker-compose.dev.yml"
	@echo ""

	@echo "$(YELLOW)🐘 BASE DE DATOS (PROD & DEV)$(RESET)"
	...
	@echo "  make db-dump1                 → Backup de Producción con variables .env"
	...
	@echo "  make clean-temp               → Eliminar CSV temporal del contenedor de base de datos"
	@echo "  make volume-ls                → Listar volúmenes de Docker"
	@echo "  make volume-inspect           → Ver metadatos del volumen sib_db_data"
	@echo ""

	@echo "$(YELLOW)🌐 REDES Y DIAGNÓSTICO DE COMPONENTES$(RESET)"
	@echo "  make networks-ls              → Listar las redes virtuales de Docker"
	@echo "  make network-inspect          → Inspeccionar miembros e IPs de una red específica"
	@echo "  make ping-db                  → Diagnóstico: Conectividad interna Backend -> DB"
	...
	@echo "  make docker-gateway           → Obtener IP del Gateway de la red del VPS"
	@echo "  make docker-ip                → Obtener IP interna de contenedor (Ej: make docker-ip NAME=...)"
	@echo ""

	@echo "$(YELLOW)🧹 LIMPIEZA Y APAGADO EN VPS$(RESET)"
	@echo "  make down-all                 → Detener y remover entorno de Producción"
	@echo "  make down-dev                 → Detener y remover entorno de Desarrollo"
	@echo "  make down-volumes             → Detener Producción y ELIMINAR VOLÚMENES FÍSICOS (Peligro)"
	@echo "  make down-dev-volumes         → Detener Desarrollo y ELIMINAR VOLÚMENES FÍSICOS (Peligro)"
	@echo "  make clean-images             → Remover imágenes Docker huérfanas / en desuso"
	@echo "  make prune                    → Limpieza segura del motor de Docker"
	@echo "  make network-prune            → Eliminar redes virtuales sin uso"
	@echo "  make clean-danger             → HARD RESET: Elimina contenedores, imágenes y volúmenes"
	@echo ""

	@echo "$(YELLOW)🛠️ CONSTRUCCIÓN LOCAL / CI-CD LOCAL (PROD & DEV)$(RESET)"
	@echo "  make build-local              → Compilar imágenes de Producción localmente"
	@echo "  make build-backend-local      → Compilar Backend PROD localmente"
	@echo "  make build-admin-local        → Compilar Frontend Admin PROD localmente"
	@echo "  make build-public-local       → Compilar Frontend Público PROD localmente"
	@echo "  make build-dev-local          → Compilar imágenes de Desarrollo localmente (.env.dev)"
	@echo "  make build-dev-backend-local  → Compilar Backend DEV localmente"
	@echo "  make build-dev-admin-local    → Compilar Frontend Admin DEV localmente (.env.dev)"
	@echo "  make build-dev-public-local   → Compilar Frontend Público DEV localmente (.env.dev)"
	@echo ""

	@echo "$(YELLOW)🔗 CONEXIONES REMOTAS Y TÚNELES SSH$(RESET)"
	@echo "  make ssh-vps                  → Conexión SSH interactiva al VPS"
	@echo "  make proxy-tunnel             → Túnel SSH para Nginx Proxy Manager (http://localhost:8080)"
	@echo "  make db-tunnel                → Túnel SSH para BD de PRODUCCIÓN (localhost:5433)"
	@echo "  make db-dev-tunnel            → Túnel SSH para BD de DESARROLLO (localhost:5434)"
	@echo "=================================================================="